...
function buildShareText(){
  ...
  SEM_ORDER.filter(([y,s])=>semHistory[y+'|'+s]).forEach(([year,sem])=>{
    ...
    lines.push(`── ${semLabel(key)}  (GPA: ${semHistory[key].gpa.toFixed(2)}) ──`);
    const allCourses=[
...
function _buildExportSemBlocks() {
  ...
  savedSems.forEach(([year, sem]) => {
    ...
    semBlocks.push({ sem, key, courses: filteredCourses });
  });
...
window.exportAsImage = async function() {
  ...
  try {
    ...
    semBlocks.forEach(({ sem, key, courses }) => {
      ctx.font = `500 9px ${FONT}`;
      ctx.fillStyle = MUT;
      ctx.fillText(semLabel(key).toUpperCase(), PAD, y + 12);
...
function buildTranscriptCsv() {
  ...
  const presets = getCoursePresets();
  const electives = getElectivePresets();
  const rows = [['Department', 'Profile']];
  rows.push([activeDept, name]);
  rows.push([]);
  rows.push(['Term', 'Course', 'Credits', 'Grade', 'Points']);
  SEM_ORDER.filter(([y, s]) => semHistory[y + '|' + s]).forEach(([year, sem]) => {
    const key = year + '|' + sem;
    const dataKey = activeDept + '|' + key;
    const saved = semData[dataKey] || [];
    ...
    const allCourses = [
      ...
      ...elects.map((nm, j) => {
        const idx = preset.length + j;
        return { name: nm, cr: saved[idx]?.credits || 3, grade: saved[idx]?.grade || '', isZero: false };
      }),
    ];
    saved.slice(preset.length + elects.length).forEach(extra => {
      if (extra?.retake && extra.name) allCourses.push({ name: extra.name, cr: extra.credits || 3, grade: extra.grade || '', isZero: false });
    });
    allCourses.forEach(({ name: courseName, cr, grade, isZero }) => {
      if (grade === 'SKIP') return;
      rows.push([
        semLabel(key),
        courseName,
        isZero ? '' : cr,
        isZero ? (grade || '') : (grade || ''),
        (!isZero && grade && GRADE_POINTS[grade] !== undefined) ? GRADE_POINTS[grade] : '',
      ]);
    });
    rows.push([]);
  });
  const cum = calcCumulative(semHistory);
  if (cum) rows.push(['Cumulative GPA', '', '', cum.val, '']);
  return rows.map(r => r.map(_csvCell).join(',')).join('\r\n');
}

window.exportCsv = function() {
  if (!Object.keys(semHistory).length) { showToast('No saved semesters to export'); return; }
  const csv = buildTranscriptCsv();
  const filename = activeDept + '_GPA.csv';

  if (typeof Android !== 'undefined' && Android.exportFile) {
    try {
      Android.exportFile(csv, filename);
      return;
    } catch (e) {
      console.warn('Android exportFile failed:', e);
    }
  }
  ...
  try {
    ...
    if (navigator.clipboard && navigator.clipboard.writeText) {
      navigator.clipboard.writeText(csv)
        .then(() => showToast('Copied CSV to clipboard (download not available)'))
        .catch(() => showToast('CSV export failed'));
    } else {
